			...
			for (int j = 0; j < 6; j++) {
				for (int i = 0; i < 9; i++) {
					for (int v = 1; v < 3; v++) {
						...
						dat.x = _boss->getX();
						dat.y = _boss->getY();
						dat.ang = Calculator::getIns()->getAngleToPlayer(dat.x, dat.y) - PI / 40 * 4 + PI / 40 * i + PI2 / 6 * j;
						dat.id = 0;
						dat.v = 4 + i + 0.79*v;
...
void BarrageHard::Barrage028()
{
	...
	for (auto l : *laserList) {
		...
		if (id == 0) {
			...
			if (80 <= cnt && cnt <= 180 && (cnt % 5 == 0 || cnt % 5 == 2 || cnt % 5 == 4)) {
				int t = cnt - 80;
				{
					bullet_t dat;
					...
					dat.id = 0;
					dat.type = eBulletType::eStar;
					dat.color = cnt%eBulletColor::eColorNum;
					_list.push_back(new Bullet(&dat));
...
void BarrageHard::Barrage030()
{
	if ((_counter % 400) == 400 - 60) {
		_boss->moveBitMiddle(30);
	}

	if (_counter % 12 == 0 && _counter % 400 <= 240) {
	...
	for (auto l : *laserList) {
		...
		if (id == 0) {
			...
			if (t == 320) {
				for (int i = 0; i < 70; i++) {
					bullet_t dat;
					dat.x = l->startpt.x + cos(l->angle)*l->length*i / 70;
					dat.y = l->startpt.y + sin(l->angle)*l->length*i / 70;
					dat.ang = randf2(PI);
					dat.v = 0;
					dat.id = 0;
					dat.isAddBlend = true;
		...
		if (_counter % 360 == 0 + 50 * t || _counter % 360 == 5 + 50 * t || _counter % 360 == 10 + 50 * t || _counter % 360 == 15 + 50 * t || _counter % 360 == 20 + 50 * t) {
			for (int j = 0; j < 8; j++) {
				for (int i = 0; i < 9; i++) {
					...
					dat.x = _boss->getX();
					dat.y = _boss->getY();
					dat.ang = _baseAngle[0] - PI / 25 * 4 + PI / 25 * i + PI2 / 8 * j;
					dat.id = 0;
					dat.v = 5 + 0.47*i;
					dat.type = eBulletType::eStar;
					dat.color = eBulletColor::eBlue + i%eBulletColor::eColorNum;
...
void BarrageHard::Barrage032()
{
	if (_counter % 40 == 0) {
		...
		for (int j = 0; j < 20; j++) {
			...
			for (int i = 0; i < 9; i++) {
				...
				dat.x = _boss->getX();
				dat.y = _boss->getY();
				dat.ang = angle + PI / 150 * i + PI2 / 20 * j;
				dat.id = 0;
				dat.v = 1.4 + 20;
				dat.a = 64;
				dat.type = eBulletType::eStar;
	...
	/*
	if (_boss->getResidualHp() < 60 && _counter % 40 == 15) {
		float angle = randf2(PI);
		for (int j = 0; j < 6; j++) {
			int color = GetRand(eBulletColor::eColorNum - 1);
			for (int i = 0; i < 9; i++) {
				...
				dat.x = _boss->getX();
				dat.y = _boss->getY();
				dat.ang = angle + PI / 180 * i + PI2 / 6 * j;
				dat.id = 0;
				dat.a = 64;
				dat.v = 1.4 + 20;
				dat.type = eBulletType::eStar;
	...
	*/
	if (_boss->getResidualHp() < 75 && _counter % 120 == 0) {
		float angle = Calculator::getIns()->getAngleFromBossToPlayer();
		for (int i = 0; i < 10; i++) {
			bullet_t dat;
			dat.x = _boss->getX();
...
void BarrageHard::Barrage033()
{
	if (_counter % 120 == 0) {
		_boss->moveBit(60);
	}
	if (_counter % 240<120 && _counter % 9 == 0) {
		int t = _counter % 240;
...
void BarrageHard::Barrage034()
{
	...
	if (_boss->getResidualHp() >= 66) {
		...
		if (_counter > 50 && _counter % 1 == 0) {
			for (int i = 1; i < 6; i++) {
				bullet_t dat;
				dat.ang = PI2 / 5 * i - PI2 / 900 * _counter;
				dat.x = _boss->getX() + cos(dat.ang) * 800;
				dat.y = _boss->getY() + sin(dat.ang) * 800;
				dat.id = 0;
				dat.v = -2.5;
		...
		if (_counter % 2 == 0) {
			...
			for (int i = 1; i < 6; i++) {
				bullet_t dat;
				dat.ang = PI2 / 5 * i - PI2 / 637 * _counter;
				dat.x = _boss->getX() + cos(dat.ang) * 800;
				dat.y = _boss->getY() + sin(dat.ang) * 800;
				dat.id = 1;
				dat.v = -8;
		...
		}
		{
			if (_boss->getResidualHp() < 33 && _counter % 30 == 0) {
				for (int i = 0; i < 12; i++) {
					...
					dat.x = _boss->getX();
					dat.y = _boss->getY();
					dat.ang = PI2 / 12 * i + randf2(PI2 / 20);
					dat.id = 4;
					dat.v = 2.5 + randf2(1);
...
void BarrageHard::Barrage035()
{
	if (_counter % 100 == 0) {
		_boss->moveBit(60);
	}
	if (_counter % 240<100 && _counter % 9 == 0) {
		int t = _counter % 240;
	...
	if (100 <= _counter % 240 && _counter % 240<200 && _counter % 10 == 0) {
...
void BarrageHard::Barrage036()
{
	...
	for (auto b : _list) {
		if (b->dat.id < 100) {
			...
			if (b->dat.counter % 150 == (b->dat.id * 3 + 40) % 150) {
				b->dat.a = 24;
				b->dat.hit = false;
			}
		}
...
void BarrageHard::Barrage038()
{
	...
	if (_n <= 2) {
		if (_counter % 1200 < 400 && _counter % 1 == 0) {
			if (_counter % 1200 == 0) {
		...
		if (1<= b->dat.id && b->dat.id<10) {
			...
			if (_n == 1 && b->dat.counter == 90) {
				...
				for (int i = 0; i < 4; i++) {
					...
					dat.x = b->dat.x;
					dat.y = b->dat.y;
					dat.ang = PI2 / 50 * _counter - PI*0.5 + PI2*i/4;
					dat.id = 10;
					dat.v = 0;
					dat.val = 0.03;
					dat.type = eBulletType::eStar;
