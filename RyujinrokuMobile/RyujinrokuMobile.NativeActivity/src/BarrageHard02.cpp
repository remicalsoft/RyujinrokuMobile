...
void BarrageHard::Barrage006()
{
	if (_counter % 300 == 0 || _counter % 300 == 10 || _counter % 300 == 20) {
		float angle = randf2(PI);
		for (int i = 0; i < 50; i++) {
			bullet_t dat;
			dat.x = _boss->getX();
			dat.y = _boss->getY();
			dat.ang = angle + PI * 2 / 50 * i;
			dat.v = 10;
			dat.id = 0;
	...
	if (_counter % 300 == 120 || _counter % 300 == 130 || _counter % 300 == 140) {
		float angle = randf2(PI);
		for (int i = 0; i < 50; i++) {
			bullet_t dat;
			dat.x = _boss->getX();
...
void BarrageHard::Barrage008()
{
	...
	if (_counter % 700 == Launcher::TIME + 60) {
		bullet_t dat;
	...
	if (_counter % 700 == Launcher::TIME + 60 + 350) {
		bullet_t dat;
		dat.x = _boss->getX() - 200;
		dat.y = _boss->getY() + 500;
		dat.ang = -PI / 2;
...
void BarrageHard::Barrage009()
{
	if (_counter % 600 < 150) {
		...
		for (int i = 0; i < 2; i++) {
			...
			dat.x = _boss->_x;
			dat.y = _boss->_y;
			dat.ang = PI / 2 - PI2 / 100 - PI2 / 100 * t + PI*i;
			dat.v = 10;
			dat.id = 0;
			dat.type = eBulletType::eBarrier;
			dat.color = eBulletColor::eBlue;
	...
	if (300 <= _counter % 600 && _counter % 600 < 450) {
		int t = _counter % 600;
		for (int i = 0; i < 2; i++) {
			bullet_t dat;
			dat.x = _boss->_x;
...
void BarrageHard::Barrage012()
{
	...
	if (Launcher::TIME + 50 <= _counter % 1100 && _counter % 1100 <= Launcher::TIME + 50 + 420) {
		if (_counter % 50 == Launcher::TIME) {
			int r = GetRand(2);
			bullet_t dat;
	...
	if (850 <= _counter % 1100 && _counter % 1100 < 950) {
		if (_counter % 10 == 0) {
			...
			for (int i = 0; i < 70; i++) {
				...
				dat.y = _boss->getY() - 50;
				dat.ang = Calculator::getIns()->getAngleToPlayer(dat.x, dat.y) + PI2 / 70 * i;
				dat.v = 10;
				dat.id = 2;
				dat.type = eBulletType::eBarrier;
				dat.color = eBulletColor::ePink;
	...
	for (auto b : _list) {
		...
		if (b->dat.id == 1) {
			...
			if (140<b->dat.counter&&b->dat.counter < 140 + 140) {
				b->dat.v += b->dat.val;
			}
		}
	}
}
void BarrageHard::Barrage013()
{
	if (0 <= _counter % 300 && _counter % 300 < 100) {
		if (_counter % 10 == 0) {
			...
			for (int i = 0; i < 40; i++) {
				...
				dat.x = _boss->getX() - 400 + 80 * t;
				dat.y = _boss->getY() - 50;
				dat.ang = Calculator::getIns()->getAngleToPlayer(dat.x, dat.y) + PI2 / 40 * i;
				dat.v = 7;
				dat.id = 2;
				dat.type = eBulletType::eBarrier;
				dat.color = eBulletColor::ePink;
	...
	if (150 <= _counter % 300 && _counter % 300 < 250) {
		if (_counter % 10 == 0) {
			...
			float ang = randf2(PI / 10);
			for (int i = 0; i < 30; i++) {
				bullet_t dat;
				dat.x = _boss->getX() - 400 + 80 * t;
				dat.y = _boss->getY() - 50;
				dat.ang = ang + PI2 / 30 * i;
				dat.v = 7;
				dat.id = 2;
				dat.type = eBulletType::eBarrier;
				dat.color = eBulletColor::eBlue;
...
void BarrageHard::Barrage014()
{
	if (_counter == 0) {
		for (int i = 0; i < 87; i++) {
			_launcher.push_back(new Launcher(_boss->getX(), _boss->getY(), -428 + 10 * i, -120, eLauncherType_Hanabi));
		}
	}
	...
	if (_counter>180 && _counter%120<20 && _counter%3) {
		for (int i = 0; i < 40; i++) {
			bullet_t dat;
			dat.x = _boss->getX();
			dat.y = _boss->getY();
			dat.ang = Calculator::getIns()->getAngleFromBossToPlayer()+PI2/40*i;
			dat.v = 7;
			dat.id = 100;
			dat.isAddBlend = true;
			dat.type = eBulletType::eBarrier;
	...
	if (_boss->getResidualHp() < 50) {
		if (_remCounter == 0) {
			_launcher.push_back(new Launcher(_boss->getX(), _boss->getY(), 0, +500, _launcherImage[0]));
		}
			...
			if (b->dat.counter == 170) {
				if (GetRand(5) == 0) {
					...
					for (int i = 0; i < 1; i++) {
						...
						dat.x = b->dat.x;
						dat.y = b->dat.y;
						dat.ang = PI / 2 + randf2(PI / 20);
						dat.v = b->dat.v + randf2(0.4);
						dat.id = 1;
						dat.type = eBulletType::eSmall;
						dat.color = b->dat.color;
						_list.push_back(new Bullet(&dat));
