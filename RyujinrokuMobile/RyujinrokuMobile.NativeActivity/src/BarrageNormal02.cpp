...
void BarrageNormal::Barrage008()
{
	...
	if (_counter % 900 == Launcher::TIME + 60) {
		bullet_t dat;
	...
	if (_counter % 900 == Launcher::TIME + 60 + 450) {
		bullet_t dat;
		dat.x = _boss->getX() - 200;
		dat.y = _boss->getY() + 500;
		dat.ang = -PI / 2;
...
{
	if (_counter % 600 < 150 && _counter % 2) {
		int t = _counter % 600;
		bullet_t dat;
		dat.x = _boss->_x;
		dat.y = _boss->_y;
...
void BarrageNormal::Barrage012()
{
	...
	for (auto b : _list) {
		...
		if (b->dat.id == 1) {
			...
			if (140<b->dat.counter&&b->dat.counter < 140 + 90) {
				b->dat.v += b->dat.val;
			}
		}
	}
	...
	if (0 <= _counter % 300 && _counter % 300 < 100) {
		if (_counter % 10 == 0) {
			...
			for (int i = 0; i < 20; i++) {
				...
				dat.x = _boss->getX() - 400 + 80 * t;
				dat.y = _boss->getY() - 50;
				dat.ang = Calculator::getIns()->getAngleToPlayer(dat.x, dat.y) + PI2 / 20 * i;
				dat.v = 7;
				dat.id = 2;
				dat.type = eBulletType::eBarrier;
				dat.color = eBulletColor::ePink;
