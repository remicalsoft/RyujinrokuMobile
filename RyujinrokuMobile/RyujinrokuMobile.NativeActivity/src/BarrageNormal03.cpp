	...
	if (_counter % 200 == 0) {
		for (int j = 0; j < 10; j++) {
			for (int i = 0; i < 60; i++) {
				bullet_t dat;
				dat.ang = PI2 / 60 * i;
				dat.x = _boss->getX() + cos(dat.ang) * 180;
				dat.y = _boss->getY() + sin(dat.ang) * 180;
...
void BarrageNormal::Barrage017()
{
	...
	if (60<_counter % 300 && _counter % 80 == 70) {
		for (int i = 0; i < 20; i++) {
			bullet_t dat;
			dat.ang = PI2 / 20 * i;
			dat.x = _boss->getX();
	...
	if (_counter % 300 == 60) {
		for (int i = 0; i < 20; i++) {
			bullet_t dat;
			dat.ang = PI2 / 20 * i;
			dat.x = _boss->getX() - 120 + cos(dat.ang) * 140;
			dat.y = _boss->getY() + 0 + sin(dat.ang) * 140;
	...
	if (_counter % 300 == 120) {
		for (int i = 0; i < 20; i++) {
			bullet_t dat;
			dat.ang = PI2 / 20 * i;
			dat.x = _boss->getX() - 180 + cos(dat.ang) * 140;
			dat.y = _boss->getY() + 0 + sin(dat.ang) * 140;
	...
	if (_counter % 300 == 180) {
		for (int i = 0; i < 20; i++) {
			...
			dat.ang = PI2 / 20 * i;
			dat.x = _boss->getX() - 120 + cos(dat.ang) * 140;
			dat.y = _boss->getY() - 120 + sin(dat.ang) * 140;
			dat.baseX = _boss->getX() - 120;
	...
	if (_counter % 300 == 0) {
		...
		for (int j = 0; j < 2; j++) {
			for (int i = 0; i < 8; i++) {
				...
				laser->lphy.angle = PI2 / 8 + PI2 / 8 * j;
...
void BarrageNormal::Barrage024()
{
	...
	if (_counter % 6 == 0) {
		for (int i = 0; i < 2; i++) {
			bullet_t dat;
			dat.ang = PI2 / 180 * _counter + PI2 / 2 * i;
			dat.x = _boss->getX();
			dat.y = _boss->getY();
			dat.v = 0;
			dat.id = 1;
