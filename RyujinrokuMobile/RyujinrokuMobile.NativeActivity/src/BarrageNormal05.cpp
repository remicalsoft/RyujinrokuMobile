...
void BarrageNormal::Barrage039()
{
	...
	if (_counter % 300 <= 80 && _counter % 2 == 0) {
		int t = _counter % 300;
		if (t == 0) {
	...
	if (150 <= _counter % 300 && _counter % 300 <= 150 + 80 && _counter % 2 == 0) {
...
void BarrageNormal::Barrage040()
{
		...
		for (int l = 0; l < 2; l++) {
			...
			for (int i = 0; i < 10; i++) {
				for (int s = 0; s < 40; s++) {
					...
					dat.x = p->x;
					dat.y = p->y;
					dat.ang = ang + PI2 / 10 * i + PI2 / 1400 * s;
					dat.id = 0;
					dat.isAddBlend = true;
	...
	if (_boss->getResidualHp()<50) {
		if (_baseAngle[0] == 0 || _counter % 120 == 0) {
		...
		if (_counter % 120 < 10 && _counter % 1 == 0) {
			for (int j = 0; j < 2; j++) {
				for (int i = 0; i < 20; i++) {
					bullet_t dat;
...
void BarrageNormal::Barrage041()
{
	...
	if (_counter % 400 <= 120 && _counter % 2 == 0) {
		...
		for (int i = 0; i < 20; i++) {
			...
			dat.x = cos(PI*0.2)*t * 10;
			dat.y = sin(PI*0.2)*t * 10;
			dat.ang = PI2 / 20 * i + _baseAngle[0] + PI / 180 * t;
			dat.id = 0;
			dat.v = 4;
			dat.type = eBulletType::eKome;
			dat.color = eBulletColor::eBlue;
...
void BarrageNormal::Barrage043()
{
	...
	if (_counter % 400 <= 100 && _counter % 5 == 0) {
		int t = _counter % 400;
		if (t == 0) {
			_baseAngle[0] = randf2(PI);
		}
		for (int i = 0; i < 14; i++) {
			...
			dat.x = cos(PI*0.2)*t * 10;
			dat.y = sin(PI*0.2)*t * 10;
			dat.ang = PI2 / 14 * i + _baseAngle[0] + PI / 90 * t;
			dat.id = 0;
			dat.v = 3.5;
			dat.type = eBulletType::eKome;
			dat.color = eBulletColor::eBlue;
	...
	if (200 <= _counter % 400 && _counter % 400 <= 200 + 100 && _counter % 5 == 0) {
		int t = (_counter % 400) - 200;
		if (t == 0) {
			_baseAngle[0] = randf2(PI);
		}
...
{
	if (_counter % 120 < 60 && _counter % 2 == 0) {
		if (_counter % 120 == 0) {
		...
		for (int j = 0; j < 2; j++) {
			for (int i = 0; i < 30; i++) {
				...
				dat.x = _boss->getX();
				dat.y = _boss->getY();
				dat.ang = _baseAngle[1] + PI2 / 30 * i + PI2 / 60;
				dat.id = j;
				dat.v = 5.8;
				dat.isAddBlend = true;
				dat.type = eBulletType::eKome;
	...
	/*
	if (120 <= _counter % 240 && _counter % 240 < 180 && _counter % 2 == 0) {
	bullet_t dat;
	dat.x = GetRand(OUT_W);
	dat.y = -20;
	dat.ang = PI*0.6;
	...
	for (int j = 0; j < 2; j++) {
		if ((T / 2)*j <= _counter % T && _counter % T <= (T / 2)*j + 20 && _counter % 1 == 0) {
			...
			for (int i = 0; i < 17; i++) {
				...
				dat.x = _boss->getX();
				dat.y = _boss->getY();
				dat.ang = PI2 / 17 * i + _baseAngle[0] + PI / 250 * t*(j ? -1 : 1);
				dat.id = 0;
				dat.v = 4;
				dat.isAddBlend = true;
				dat.type = eBulletType::eKome;
	...
	if (_counter>60) {
		...
		if (t % 4 == 0) {
			for (int j = 0; j < 2; j++) {
				for (int i = 0; i < 18; i++) {
					bullet_t dat;
					dat.ang = PI2 / 80 * i + PI2 / 360 * t + (j ? PI : 0);
					dat.x = _boss->getX() + cos(dat.ang) * 50;
					dat.y = _boss->getY() + sin(dat.ang) * 50;
					dat.v = 8;
					dat.id = 1;
...
void BarrageNormal::Barrage048()
{
	if (_counter % 2 == 0) {
		for (int i = 0; i < 28; i++) {
			...
			dat.x = _boss->getX();
			dat.y = _boss->getY();
			dat.ang = PI2 / 1200 * _counter + PI2 / 28 * i;
			dat.id = 0;
			dat.v = 8;
			dat.isAddBlend = true;
			dat.type = eBulletType::eKome;
...
void BarrageNormal::Barrage052()
{
	if (_boss->getResidualHp()>50) {
		if (_counter % 280 == 0) {
			...
			for (int i = 0; i < 8; i++) {
				bullet_t dat;
				dat.x = Calculator::getIns()->getPlayerX() + cos(PI2 / 8 * i + ang) * 150;
				dat.y = Calculator::getIns()->getPlayerY() + sin(PI2 / 8 * i + ang) * 150;
				dat.ang = PI2 / 8 * i + ang + PI;
			...
			for (int j = 0; j < 5; j++) {
				...
				for (int i = 0; i < 8; i++) {
					bullet_t dat;
					dat.x = Calculator::getIns()->getPlayerX() + cos(PI2 / 8 * i + angle) * 30 + cos(PI2 / 5 * j + ang) * 300;
					dat.y = Calculator::getIns()->getPlayerY() + sin(PI2 / 8 * i + angle) * 30 + sin(PI2 / 5 * j + ang) * 300;
					dat.ang = PI2 / 8 * i + angle + PI;
					dat.id = _counter == 0 ? 0 : 1;
					dat.isAddBlend = true;
					dat.v = 0;
					dat.keepAlive = true;
		...
		if (_counter2 % 240 == 60) {
			...
			for (int j = 0; j < 6; j++) {
				...
				for (int i = 0; i < 8; i++) {
					bullet_t dat;
					dat.x = Calculator::getIns()->getPlayerX() + cos(PI2 / 8 * i + angle) * 30 + cos(PI2 / 6 * j + ang) * 300;
					dat.y = Calculator::getIns()->getPlayerY() + sin(PI2 / 8 * i + angle) * 30 + sin(PI2 / 6 * j + ang) * 300;
					dat.ang = PI2 / 8 * i + angle + PI;
					dat.id = _counter == 0 ? 0 : 1;
					dat.isAddBlend = true;
					dat.v = 0;
...
void BarrageNormal::Barrage055()
{
	...
	int WaveEachTime[BARRAGE055_WAVENUM] = { /*1250,*/1200,1220,1100,1320,1200,1600,1200 };
	...
	if (_counter < WaveTime[0])
	{
		...
		double deltaAngle = 0.025;	//1Fで回転する角度
	...
	*/
	...
	{
		int count = _counter;	//このウェーブが始まってからの時間
		double deltaAngle = 0.023;	//1Fで回転する角度
		int range = 450;	//中心からの距離
		int preTime = 120;	//ランチャーが発射されてから弾を打つまでの時間
		int shotTime = 500;	//弾を撃ちおわる時間
		int endTime = 700;	//ランチャーが消える時間
		...
		for (Launcher* pl : _launcher) {
			...
			if (count >= preTime && count < shotTime)
			{
				...
				{
					if (pl->counter % 40 < 15 && pl->counter % 4 == 0)
					{
	...
	else if (_counter < WaveTime[1])
	{
		...
		for (Launcher* pl : _launcher) {
			...
			{
				{
					if (pl->counter % 18 <= 2 && pl->counter % 1 == 0) {
						bullet_t dat;
						dat.x = pl->x;
						dat.y = pl->y;
						dat.ang = Utils::getAngle(dat.x, dat.y, CENTER_X, CENTER_Y);
	...
	else if (_counter < WaveTime[2])
	{
		...
		for (Launcher* pl : _launcher) {
			...
			if (count >= preTime && count < shotTime)
			{
				if (pl->counter % 4 == 0 && pl->counter % 30 <13) {
					bullet_t dat;
					dat.x = pl->x;
					dat.y = pl->y;
	...
	else if (_counter < WaveTime[4])
	{
		...
		for (Launcher* pl : _launcher) {
			...
			if (count >= preTime && count < shotTime)
			{
				if ((count - preTime) % 3 == 0 && (count - preTime) % 100 <80) {
					bullet_t dat;
					dat.x = pl->x;
					dat.y = pl->y;
				...
				if (count >= preTime && count < shotTime)
				{
					...
					if (pl->id % 2 == 1)
					{
						if (pl->counter % 30 < 12 && pl->counter % 3 == 0) {	//ギリギリ当たらない列
							...
							dat.x = pl->x;
							dat.y = pl->y;
							dat.ang = Utils::getAngle(dat.x, dat.y, CENTER_X, CENTER_Y) - PI / 3 * 0.1 - PI / 3 * 0.9*double(count - preTime) / (shotTime - preTime);
							dat.id = (count - preTime) / 2;
							dat.v = 2;
							dat.type = eBulletType::eBarrier;
							dat.keepAlive = true;
